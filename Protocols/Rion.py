...
class Rion:
    ...
    __frame_head_length = 5
    __frame_foot_length = 3

    @staticmethod
    def encode(packet_data: bytes, packet_cmd: int):
        sync_and_start = bytes((0xff, 0x02))
        to_be_checked = struct.pack('>BH', packet_cmd, len(packet_data)) + packet_data
        crc16_kermit = Rion.__calculate_crc16_kermit(to_be_checked)
        crc16_kermit_and_end = struct.pack('>HB', crc16_kermit, 0x03)

        return sync_and_start + to_be_checked + crc16_kermit_and_end

    @staticmethod
    def decode(buffer: bytearray, errors: int):
        packet_id = None
        packet_data = None
        while len(buffer) >= Rion.__frame_head_length:
            # Find the start of a packet by scanning for a valid data head
            if (buffer[0] != 0xff) or (buffer[1] != 0x02):
                del buffer[0]
                continue
            # Calculate the packet_data length and compare it with data packets
            packet_data_length = buffer[3] << 8 | buffer[4]
            packet_length = Rion.__frame_head_length + Rion.__frame_foot_length + packet_data_length
            if len(buffer) < packet_length:
                break
            # Check crc16_kermit of the buffer
            if (buffer[packet_length - 3] << 8 | buffer[packet_length - 2]) != Rion.__calculate_crc16_kermit(
                    bytes(buffer[2:(packet_length - 3)])):
                del buffer[0]
                errors += 1
                continue
            packet_id = buffer[2]
            packet_data = buffer[Rion.__frame_head_length:Rion.__frame_head_length + packet_data_length]
    ...
    def __calculate_crc16_kermit(data: bytes):
        crc16 = 0
        for item in data:
            crc16 = (crc16 >> 8 ^ Rion.__crc16_tuple[crc16 & 0xff ^ item]) & 0xffff
        return crc16
